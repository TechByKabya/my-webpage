...
/* ─── component ───────────────────────────────────────── */
export const ProjectPostHeader: React.FC = () => {
  ...
  const tagField        = useFormFields(([f]) => f['tag'])
  const descField       = useFormFields(([f]) => f['description'])
  const linkUrlField    = useFormFields(([f]) => f['linkUrl'])
  const gridSpanField   = useFormFields(([f]) => f['gridSpan'])
  const isGithubField   = useFormFields(([f]) => f['isGithubCard'])
  ...
  const tag         = (tagField?.value      as string)  || ''
  const linkUrl     = (linkUrlField?.value  as string)  || ''
  const gridSpan    = (gridSpanField?.value as string)  || 'span-1'
  const isGithub    = (isGithubField?.value as boolean) || false
  ...
  const handleCopySlug = useCallback(async () => {
    if (!slug) return
    await navigator.clipboard.writeText(`/projects/${slug}`)
    setCopied(true)
  ...
  return (
    <div style={{
      ...
      <style>{`
        .pph-btn { transition: all 0.18s ease; }
        .pph-btn:hover { opacity: 0.85; transform: translateY(-1px); }
        .pph-btn:active { transform: translateY(0); }
        .pph-card { transition: box-shadow 0.2s; }
        .pph-card:hover { box-shadow: 0 8px 24px rgba(0,0,0,0.08) !important; }
      `}</style>

      {/* ── Top Header Banner ── */}
      <div style={{
        background: 'linear-gradient(135deg, #0c1a2e 0%, #162032 50%, #0c1a2e 100%)',
        borderRadius: '20px',
        padding: '28px 32px',
        marginBottom: '20px',
        boxShadow: '0 8px 32px rgba(12,26,46,0.25)',
        position: 'relative',
        overflow: 'hidden',
      }}>
        {/* Decorative glow */}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '24px', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: '240px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px', flexWrap: 'wrap' }}>
              ...
              <span style={{
                ...
                {gridSpan === 'span-2' ? <LayoutGrid size={10} /> : <LayoutTemplate size={10} />}
                {gridSpan === 'span-2' ? 'Wide (2-col)' : 'Standard'}
              </span>
            </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                ...
                {linkUrl && (
                  <a
                    ...
                    style={{
                      display: 'inline-flex', alignItems: 'center', gap: '4px',
                      color: '#60a5fa', textDecoration: 'none', fontSize: '0.78rem',
                    }}
                  >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'flex-end' }}>
            {id && slug && (
              <a
                href={`https://www.kabyac.tech/projects/${slug}`}
                target="_blank"
                rel="noopener noreferrer"
                className="pph-btn"
                style={{
      ...
      <div style={{
        ...
        <div className="pph-card" style={{
          ...
          <div>
            <div style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>SEO Score</div>
        ...
        <div className="pph-card" style={{
          ...
          <div>
            <div style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>Title Length</div>
        ...
        <div className="pph-card" style={{
          ...
          <div>
            <div style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>Description</div>
            <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0f172a' }}>
        ...
        <div className="pph-card" style={{
          ...
          <div>
            <div style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>Word Count</div>
            <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0f172a' }}>
      ...
      {score < 50 && !isNew && (
        <div style={{
          ...
          <span style={{ fontSize: '0.85rem', color: '#92400e', fontWeight: 500 }}>
            <strong>SEO Tip:</strong> Save this project to auto-generate meta tags via AI, or fill them in the sidebar manually for a better search ranking score.
          </span>
        </div>
      )}
