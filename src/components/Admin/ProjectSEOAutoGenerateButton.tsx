...
export const ProjectSEOAutoGenerateButton: React.FC<{ field: UIField }> = () => {
  ...
  // Projects use "description" not "excerpt"
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const excerpt = descriptionField?.value as string || ''
      ...
      if (data.seoTitle) setMetaTitle(data.seoTitle)
      if (data.seoDescription) setMetaDescription(data.seoDescription)
    ...
    <div style={{ marginBottom: '2rem', padding: '1.5rem', background: '#f8fafc', borderRadius: '16px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'flex-start', boxShadow: '0 1px 3px 0 rgba(0,0,0,0.05)' }}>
      <div>
        ...
        <p style={{ margin: 0, color: '#64748b', fontSize: '0.875rem' }}>
          Automatically generate an optimized SEO Title and Meta Description based on your project content using Gemini AI.
        </p>
      </div>
